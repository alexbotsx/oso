...
export default async function importLoader(module) {
    ...
    //         workerData: module
    //     })
    //     const OsoWorker = () => worker.terminate().catch(() => { })
    //     worker.once('message', (msg) => (OsoWorker(), console.log(msg.data), resolve(msg)))
    //     worker.once('error', (error) => (OsoWorker(), reject(error)))
    // })
